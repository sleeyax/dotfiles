...
**Drawing with images.** The module has no text beyond spaces: `themes/ml4w-modern/style.css` paints three `background-image` layers over them. The first is `waybar/claude.svg`, a tracked asset: the Claude mark from Simple Icons, recoloured, and drawn at 18px against the rings' 16px so it reads as the label rather than a third gauge. The other two are the usage rings, which `claude-usage.sh` writes to `$XDG_CACHE_HOME/claude-usage/`. No font glyph fills continuously — the closest, `nf-md-circle_slice_1..8`, quantises to eighths — so an exact percentage has to be drawn.
